...
class ChatController extends BaseController {
    ...
    public function getheadimg()
    {
        ...
        if($fromid)
        {
            $whereuser['user_id']=array('eq',$fromid);
            $userinfo=M('user')->field('user_id,nick_name,head_img')->where($whereuser)->find();
            $head_img=$userinfo['head_img'];
            $nick_name=$userinfo['nick_name'];
            
            $wherefriend['user_id']=array('eq',$user_id);
            $wherefriend['frienduser_id']=array('eq',$fromid);
            $userinfo=M('friend')->field('istop,isdisturb,memoname')->where($wherefriend)->find();
            if($userinfo)
            {
                $istop=$userinfo['istop'];
                $isdisturb=$userinfo['isdisturb'];
                $memoname=$userinfo['memoname'];
            }
            
        }
        if($group_id)
        {
            $whereguser['y_group_user.group_id']=array('eq',$group_id);
            $whereguser['y_group_user.user_id']=array('eq',$user_id);
            $userinfo=M('group')->JOIN('LEFT JOIN y_group_user on y_group_user.group_id = y_group.group_id')->field('y_group.group_id,y_group.groupname,y_group.logo,y_group_user.istop,y_group_user.isdisturb,y_group_user.memoname')->where($whereguser)->find();
            $head_img=$userinfo['logo'];
            $nick_name=$userinfo['groupname'];
            $istop=$userinfo['istop'];
            $isdisturb=$userinfo['isdisturb'];
            $memoname=$userinfo['memoname'];
        }
        echo json_encode(array('code'=>200,'msg'=>'获取成功','head_img'=>enThumb("./Uploads",$head_img),'nick_name'=>$nick_name,'istop'=>$istop,'isdisturb'=>$isdisturb,'memoname'=>$memoname));die;
    }
...
//         date_default_timezone_set("Asia/Shanghai");
        ...
        $group_id = $_REQUEST['group_id'];
        $key = $_REQUEST['key'];
        $mykey=date("Y-m-d H:i", time()).$serverkey;
//         echo $mykey;
        $mykey=MD5($mykey);
//           echo $mykey;
        ...
        if($mykey==$key)
        {
            if($group_id)
            {
                ...
                $data=M('group_user')->where($wheregroup)->select();
                $groupdata=M('group')->where($wheregroup)->find();
                if($data){
                    echo json_encode(array('code'=>200,'msg'=>'获取成功','data'=>$data,'groupname'=>$groupdata['groupname']));die;
                }else {
                    echo json_encode(array('code'=>203,'msg'=>'群组数据为空'));die;
                }
                
            }else 
            {
                echo json_encode(array('code'=>202,'msg'=>'群组group_id不能为空'));die;
            }
            
        }else {
            echo json_encode(array('code'=>201,'msg'=>'密钥不对'));die;
...
////        $level1=array_splice($level1,$p,$num*$p);
...
//        }
        ...
        if(IS_POST){
            ...
            $mywhere['cart'] = trim($_REQUEST['cart']); //卡号

            $mywhere['institution_id'] = array("eq",0); # 除了机构 //除了机构

            $mybankinfo = M("mybank")->where($mywhere)->getField('mybank_id'); //拿到绑定的银行记录id
            ...
            $validateCode = trim($_REQUEST['code']); //鉴权短信的验证码
...

            {
                ...
                $res = $this->ymBankCard($validateCode);  //鉴权绑卡确认
...
//                # success api
                ...
                if($res['ret_code'] != 1)   # error

                {

                    echo json_encode(array('code'=>4,'msg'=>$res['ret_msg']));die;
...
//                $yzmInfo = M('mobleyzm')->where(array('phone'=>$data['mobile'],'type'=>3))->find();

//                if ($data['code'] != $yzmInfo['code']){

//                    echo json_encode(array('code'=>2,'msg'=>'验证码不正确'));die;
...
//                $msg = R('Payapi/Api/getBankRz',array($Info,$sn));

////                dump($msg);die;

//                $bankMsg['uid']=$this->uid;

//                $bankMsg['phone']=$data['mobile'];

//                $bankMsg['ordersn']=$sn;

//                $bankMsg['orgId']=$msg['responseData']['header']['orgId'];

//                $bankMsg['json']=json_encode($msg);

//                $bankMsg['uno']=trim($msg['bushiData']['records']['uno']);

//                $bankMsg['status']=$msg['responseData']['header']['status'];

//                $bankMsg['code']=$msg['responseData']['header']['code'];

//                $bankMsg['t']=time();

//                M('return_bankrz')->add($bankMsg);

//                if ($msg['responseData']['header']['status'] == 'E000000'){
                ...
                echo json_encode(array('code'=>2099,'msg'=>'错误,请重新刷新!'));die;
...
//            {"rt2_retCode":"0000","sign":"ef7cacdcd79ff6bad768542d51e3d556","rt1_bizType":"BankCardUnbind","rt4_customerNumber":"C1800001834","rt3_retMsg":"成功"}
...

    {
        ...
        return M('user')->where(array('user_id'=>$uid))->getField('phone');
...
//            echo json_encode(array('code'=>3,'msg'=>'已提交绑定'));die;
            ...
            echo json_encode(array('code'=>2016,'msg'=>'储蓄卡提交审核成功'));die;
...
//        echo json_encode(array('data'=>$msg));die;
        ...
        $mywhere['institution_id'] = array("eq",0); # 除了机构
        ...
        $validateCode = trim($_REQUEST['code']);  //验证码

        if($validateCode)

        {

            $res = $this->ymBankCard($validateCode); //返回鉴权信息
...
//                # success api
...

            {
                ...
                echo json_encode(array('code' => 4, 'msg' => $res['ret_msg']));
...
//                dump($pdata);die;
...

        {
            ...
            $ifjb['bank_children_id']=$_REQUEST['children_id']; //支行

            $ifjb['province_id']=$_REQUEST['province_id'];  //省

            $ifjb['city_id']=$_REQUEST['city_id'];//市

            $ifjb['area_id']=$_REQUEST['area_id'];//区
...
//            $ifjb['is_normal']=2;
            ...
            $data['bank_children_id']=$_REQUEST['children_id']; //支行

            $data['province_id']=$_REQUEST['province_id'];  //省

            $data['city_id']=$_REQUEST['city_id'];//市

            $data['area_id']=$_REQUEST['area_id'];//区
...
//            dump($mybankinfo);die;
...

        {
            ...
            $data['bank_id'] = trim($_REQUEST['bankid']);
...
//                    dump($zhidingM);
                ...
                $tt['status']=1;
//                dump($tt);die;
                $huafei_order = M('chongzhi_order')->field('chongzhi_order_id,user_id,fee,time,status,goodsname,type')->where($tt)->order('time desc')->select();
...
//         $data['upgrade_point']=json_encode(array(1=>'优化会员个人中心卡慢不流畅等',2=>'新增O2O收款渠道',3=>'修复安卓7.0+系统无法上传图片问题'));
//
...
//        $_SESSION['last_sign'] = $this->sign;//把sign存入session 为作判断
    ...
    $querys="bankcard=6226890118005577";
...
//        $querys = "bankcard=6226230208444123";//      6259074445765738
...
//        $querys = "bankcard=6259691108700098";//      6259074445765738
        //  6228480606400544171   zx 6226890118005577  622439880016114875
        // $querys = "bankcard=".$_REQUEST['bankcard'];
        $querys = "bankcard=6258590003722820";
        $bodys = "";
        $url = $host.$path."?".$querys;
//       dump($url);die;
        $curl = curl_init();
