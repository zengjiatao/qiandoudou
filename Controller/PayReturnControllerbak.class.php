...
//        $param['userId'] = $this->getUserId($this->uid);
        // 鉴权一次 (重新绑定)
        # 增加判断，是否存在bindId。如果不存在则重新生成一条
        $mywhere['user_id'] = $uid;
        $mywhere['type'] = 2;
        $mywhere['is_normal'] = 1;
        $mbinfo = M('mybank_bind')->where($mywhere)->find();
...
//            $card = M("mybank")->where(array('user_id'=>$uid,'type'=>1))->getField('cart');
            ...
            # 判断是否存在
            if(!empty($mbinfo['bindId']))
            {
                $mbinfobind = M('mybank_bind')->where(array('user_id'=>$uid,'is_normal'=>1,'type'=>2))->find();
                $myBind['user_id'] = $uid;
